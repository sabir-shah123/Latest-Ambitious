    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('shipping.update', $shipping->id) }}">
                ...
                <div class="form-group">
                    <label for="estimate_time" class="col-form-label">Estimated Time <span
                            class="text-danger">*</span></label>
                    <input id="estimate_time" type="text" name="estimate_time" placeholder="3 to 4 days"
                        value="{{ $shipping->estimate_time ?? '' }}" class="form-control">
                    @error('estimate_time')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
